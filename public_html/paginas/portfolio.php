
<div class="conteudo">
    <div class="conteudoTituloPerna"></div>   
    <div class="conteudoTitulo">Portfólio</div>
    <div class="conteudoMeio"><div class="disciplina">
            <a href="?pagina=design_multimidia">
                <img src="img/Dis_Multmidia.png" />
                <div class="disciplinaTexto">
                    <b>Design<br>Multmídia</b>
                </div>
            </a>
        </div>
        <div class="disciplina">
            <a href="?pagina=fotografia">
                <img src="img/Dis_fotografia.png" />
                <div class="disciplinaTexto">
                    <b>Fotografia</b>
                </div>
            </a>
        </div>
        <div class="disciplina">
            <a href="?pagina=interface_usabilidade">
                <img src="img/Dis_Interface.png" />
                <div class="disciplinaTexto">
                    <b>Interface e<br>Usabilidade</b>
                </div>
            </a>
        </div>
        <div class="disciplina">
            <a href="?pagina=animacao_grafica">
                <img src="img/Dis_animacao.png" />
                <div class="disciplinaTexto">
                    <b>Animação<br>Gráfica</b>
                </div>
            </a>
        </div>
        <div class="disciplina">
            <a href="?pagina=audio_video_digital">
                <img src="img/Dis_audio_video.png" />
                <div class="disciplinaTexto">
                    <b>Áudio e Vídeo<br>Digital</b>
                </div>
            </a>
        </div>
        <div class="disciplina">
            <a href="?pagina=culturas_midias_sociais">
                <img src="img/Dis_cultura_midias.png" />
                <div class="disciplinaTexto">
                    <b>Culturas e<br>Mídias Digitais</b>
                </div>
            </a>
        </div>
    </div>
</div>